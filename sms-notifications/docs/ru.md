...
- **Telegram-уведомления** - Отправка алертов о событиях (заказы, регистрации, отзывы) в Telegram-группу параллельно с SMS
- **OTP-подтверждение телефона** - Защита форм оформления заказа и регистрации SMS-верификацией 6-значным кодом. Поддерживает стандартный чекаут, Easy Checkout, регистрацию и универсальную защиту на уровне модели
- **Лимит повторных запросов и блокировка** - Настраиваемый лимит повторных запросов кода с блокировкой на несколько часов для предотвращения злоупотреблений
- **Вкладка "Диагностика"** - Встроенная проверка событий, подключения Telegram, конфигурации OTP и статуса шлюза
...
- Проверить наличие в **Дополнения → События** значений: `aw_sms_notify_order_alert`, `aw_sms_notify_review_alert`, `aw_sms_notify_register_alert`, `aw_sms_notify_inject_otp`
- Если OTP включен, также проверить: `aw_sms_notify_otp_register`, `aw_sms_notify_otp_checkout_std`, `aw_sms_notify_otp_checkout_easy`, `aw_sms_notify_otp_addorder`, `aw_sms_notify_otp_addcustomer`
...
На вкладке **"Telegram"** можно настроить отправку уведомлений о событиях в Telegram-группу:

- **Включить Telegram-уведомления** - Основной переключатель
- **Токен бота** - Токен от @BotFather
- **Chat ID** - Числовой ID чата (отрицательный для групп)
- **Уведомление о новом заказе** - Отправлять при оформлении нового заказа
- **Уведомление о регистрации** - Отправлять при регистрации нового покупателя
- **Уведомление о новом отзыве** - Отправлять при добавлении отзыва на товар
- **Шаблоны сообщений** - Мультиязычные HTML-шаблоны с теми же `{{переменными}}`, что и SMS. Поддерживаются теги `<b>`, `<i>`, `<a href>`.

**Инструкция по настройке:**

1. Откройте @BotFather в Telegram, отправьте `/newbot`, следуйте инструкциям, скопируйте ТОКЕН
2. Вставьте ТОКЕН в поле "Токен бота"
3. Создайте группу (или используйте существующую), добавьте бота как участника
4. Добавьте @RawDataBot в ту же группу для получения `chat_id` (отрицательное число для групп), затем удалите бота
5. Вставьте `chat_id` в поле "Chat ID"
6. Включите основной переключатель и выберите нужные события
7. Перейдите на вкладку **Диагностика** для проверки "Telegram бот доступен"

> **Примечание:** Telegram-уведомления работают параллельно с SMS - не заменяют их. Смена статуса заказа в Telegram НЕ отправляется (только SMS).

### 5. OTP-подтверждение телефона

На вкладке **"OTP"** можно включить SMS-верификацию телефона для защиты от фейковых заказов и регистраций:

- **Включить OTP** - Основной переключатель
- **Защита регистрации** - Требовать OTP на `/account/register`
- **Защита стандартного чекаута** - Требовать OTP при гостевом оформлении заказа
- **Защита Easy Checkout** - Требовать OTP в `aw_easy_checkout`
- **Универсальная защита (model-level)** - Блокировать любой вызов `addOrder`/`addCustomer` без валидной OTP-сессии. Используется как запасной вариант для сторонних модулей оформления.
- **Время жизни кода (сек)** - Как долго действителен сгенерированный код. По умолчанию: 300 (5 мин).
- **Задержка повторной отправки (сек)** - Минимальная пауза между запросами кода. По умолчанию: 30.
- **Максимум попыток ввода** - Сколько неверных кодов разрешено до сброса. По умолчанию: 5.
- **Максимум повторных запросов кода** - Сколько раз пользователь может запросить новый код. По умолчанию: 2. После превышения - блокировка.
- **Длительность блокировки (сек)** - На сколько блокируется запрос кодов после превышения лимита. По умолчанию: 7200 (2 часа).
- **Шаблон SMS** - Мультиязычный шаблон с тегом `{{code}}`.
- **Заголовок модалки / Текст модалки** - Мультиязычный текст в модальном окне подтверждения. Тег `{{phone}}` доступен в тексте модалки.

**Как это работает (пользовательский сценарий):**

1. Пользователь заполняет форму с телефоном и нажимает "Отправить"
2. JavaScript перехватывает отправку и открывает Bootstrap-модалку
3. SMS с 6-значным кодом отправляется автоматически через настроенный шлюз
4. Пользователь вводит код в модалке
5. При успехе модалка закрывается, форма отправляется с одноразовым OTP-токеном
6. Сервер проверяет токен и разрешает операцию

**Уровни защиты:**

- **Клиент:** JS перехватывает кнопки отправки на обнаруженных формах (регистрация, гостевой чекаут, easy checkout)
- **Сервер (контроллер):** Обработчики событий на `controller/account/register/before`, `controller/checkout/guest/save/before`, `controller/extension/aw_easy_checkout/validation/before`
- **Сервер (модель, универсальный):** Обработчики на `model/checkout/order/addOrder/before` и `model/account/customer/addCustomer/before` - выбрасывают исключение если универсальная защита включена и нет валидной OTP-сессии

> **Внимание:** Универсальный режим блокирует ЛЮБОЙ заказ/регистрацию через события модели. Сторонние формы без поддержки OTP JS покажут ошибку вместо модалки. Включайте только если стандартных перехватчиков недостаточно.

### 6. Настройка уведомлений Viber
...
Вкладка **"Диагностика"** предоставляет проверку состояния конфигурации модуля:

- **События:** Список всех зарегистрированных событий OpenCart и их статус (зеленый = присутствует, красный = отсутствует)
- **Telegram:** Проверяет валидность токена бота через API `getMe`, проверяет настройку chat_id
- **OTP:** Проверяет настройку SMS-шлюза, наличие OTP-шаблонов для всех языков, регистрацию событий
- **Шлюз:** Показывает текущий статус подключения шлюза

Используйте эту вкладку после установки или изменения настроек для проверки корректности конфигурации.
