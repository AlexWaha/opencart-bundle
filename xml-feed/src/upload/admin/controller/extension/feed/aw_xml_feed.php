...
class ControllerExtensionFeedAwXmlFeed extends Controller
{
    ...
    public function index()
    {
        ...
        $this->params['warranty_text'] = $this->moduleConfig->get('warranty_text', []);
        $this->params['stock_status_available'] = $this->moduleConfig->get('stock_status_available', []);

        $this->params['feed_generation_url'] = HTTPS_CATALOG . 'index.php?route=extension/feed/' . $this->moduleName . '&access_key=' . $this->params['access_key'];
    ...
    public function getForm($postData = [])
    {
        ...
        $this->params['breadcrumbs'] = [
            ...
            [
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/feed/' . $this->moduleName . '/getForm', $this->tokenData['param'] . '&type=feed', true),
            ],
        ];
        ...
        if ($feed) {
            $this->params['breadcrumbs'] = [
                'text' => $feed['name'],
                'href' => $this->url->link('extension/feed/' . $this->moduleName . '/getForm', $this->tokenData['param'] . '&feed_id=' . $feedId, true),
            ];
        }
    ...
    private function validate(): bool
    {
        ...
        if (isset($this->request->post['warranty_text'])) {
            foreach ($this->request->post['warranty_text'] as $languageId => $value) {
                if (!empty(trim($value)) && ((utf8_strlen(trim($value)) < 3) || (utf8_strlen(trim($value)) > 255))) {
                    $this->error['warranty_text'][$languageId] = $this->language->get('error_warranty_text');
                }
            }
        }
