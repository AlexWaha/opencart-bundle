    ...
    private function generateFeed($feedId)
    {
        ...
        if ($feedConfig) {
            ...
            $settings = [
                ...
                'attribute_shipping' => $feedConfig->get('attribute_shipping', 0),
                ...
                'shipping_text' => $feedConfig->get('shipping_text', []),
                'warranty_text' => $feedConfig->get('warranty_text', []),
        ...
        if ($products) {
            foreach ($products as $product) {
                ...
                if ($setting['option_size']) {
                    $optionSize = $this->getOptions($product, array($setting['option_size']), $setting['language_id']);
                } else {
                    $optionSize = [];
                }
                ...
                $shipping = false;
                if ($setting['attribute_shipping']) {
                    $shippingData = $this->getAttributes(
                        $product,
                        array($setting['attribute_shipping']),
                        $setting['language_id']
                    );
                    $shipping = ($shippingData && isset($shippingData['value'])) ? $shippingData['value'] : false;
                }

                if (!$shipping && $setting['shipping_text']) {
                    $shipping = $this->cleanTextForXml($setting['shipping_text'][$setting['language_id']] ?? '');
                }

                $warranty = false;
                if ($setting['attribute_warranty']) {
                    $warrantyData = $this->getAttributes(
                        $product,
                        array($setting['attribute_warranty']),
                        $setting['language_id']
                    );
                    $warranty = $warrantyData ? $warrantyData['value'] : false;
                }

                if (!$warranty && $setting['warranty_text']) {
                    $warranty = $this->cleanTextForXml($setting['warranty_text'][$setting['language_id']] ?? '');
                }
