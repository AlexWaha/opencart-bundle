...
/**
 * AW FAQ - Event Handler
 *
 * Injects the FAQ page URL into google_sitemap output via OpenCart event system.
...
class ControllerExtensionAwFaqEvent extends Controller
{
    public function sitemapAfter(&$route, &$data, &$output): void
    {
        ...
        $config = $this->awCore->getConfig('aw_faq');

        if (!$config->get('status', false)) {
            return;
        }

        $url = $this->url->link('extension/module/aw_faq');
